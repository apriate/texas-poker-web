...
import { ICommandList } from '@/interface/IPlayer'
...
const props = defineProps({
  gameList: {
    type: Array as PropType<IGameRecord[]>,
    default: []
  },
  commandList: {
    type: Array as PropType<ICommandList[]>,
    default: []
  },
  currGameIndex: {
    type: Number,
...
const maxIndex = computed(() => {
  return props.gameList.length
})

const getRecord = (type: number) => {
  const index = props.currGameIndex + type
  if (index > maxIndex.value || index <= 0) {
    return
  }
  emit('getRecord', index)
}

const commonCardMap = (commonCard: string) => {
      ...
      <div class="record-context">
        ...
        <ul class="record-box">
          <li v-for="player in commandList">
            ...
            <div class="commandCard">
              <CardList
                :card-list="commonCardMap(player.commonCard)"
        ...
        <div class="record-btn">
          <i
            ...
            :class="{ disable: currGameIndex === 1 }"
            @click="getRecord(-1)"
          ></i>
          <span>{{ currGameIndex }}</span>
          <i
            class="iconfont icon-arrow right"
            :class="{ disable: currGameIndex === maxIndex }"
            @click="getRecord(1)"
          ></i>
        </div>
      </div>
